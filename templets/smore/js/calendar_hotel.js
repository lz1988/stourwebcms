                        ...
                        for (var itr = 0; itr < itinerary.data.length; itr++) {
                            ...
                            if (_day == day && month == _month && _year == year) {
                                ...
                                if (flag) {
                                    ...
                                    var thhtml = "<div id='d" + itr + "' " + (this.isWeb == true ? "" : "") + " datetime='" + tempDateTime + "' exists='true' onmouseover='divchange1(this)' onmouseout='divchange2(this)' onclick=\"setBeginTime("+year+","+month+","+day+","+price+","+childprice+","+$lineid+","+$id+")\">" + dayT + day + "<div><a class='calendar_red'  >" + pricehtml + "</a><a style=\"display:block;text-align:center;\">" + itinerary.data[itr].description + "</a></div></div>";
...
function Calendar_GetPdateAndPrice_New(ele, id, url, iseven, ischangecss, isweb,func,lineid) {
    ...
    if (isweb!=undefined && isweb == false) {
        ...
        $.ajax({
            type: "post",
            url: siteUrl+"/cars/ajax.car.php",
			data: "dopost=getcarprice&suitid="+suitid,
...
//重写show
function showCalendar(containid, suitid,func,carid)
{
	Calendar_GetPdateAndPrice_New(document.getElementById(containid), suitid,"",false,null,null,func,carid);
}
