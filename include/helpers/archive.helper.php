...
function getPriceLineNum($max, $min)
{
	global $dsql;
	$destid = $GLOBALS['destid'];
	$sql = "select count(*) as dd from #@__line where ishidden='0' and price between '$min' and '$max' and FIND_IN_SET($destid,kindlist)";
	$row = $dsql->GetOne($sql);
	return $row['dd'];
}
  ...
  public static function getLoadingFile()
  {
   ...
   /**
    ...
    public static function getSlineWebInfo()
    {
        ...
        {
           $sql = "select * from #@__destinations where webprefix= '$ar[0]' ";
    ...
    public static function StatisticalOrderAmount($orderinfo)
    {
        ...
        if(is_array($orderinfo))
        {
            ...
            if($orderinfo['typeid']==2&&$orderinfo['pid']==0) {
                $numRow = $dsql->GetOne("select sum(dingnum) as num from sline_member_order where pid={$orderinfo['id']}");
                $totalPriceArr=$dsql->GetOne("select sum(dingnum*price) as totalprice from sline_member_order where pid={$orderinfo['id']}");
                $result['totalPrice']=$totalPriceArr['totalprice'];
                $result['numberDescript']=$numRow['num'];
            }
        }
        return $result;
    }
